...
- **Jinja2 everywhere:** all emission goes through editable template files + JSON maps in
  ...
  - **AUDIT 2026-08-01 — the rule is currently BROKEN, and it predates the audit.**
    ...
    `expression.py` 22 occurrences plus `_JINJA_FUNCS` (the 109-entry webCoRE-function →
    HA-template table, ~line 619); `emit_yaml.py` 17; `resolve.py` 5 (`read_expr`);
    `emit_pyscript.py` 1. Only the outer skeleton is templated — everything INSIDE the
    automation is generated by Python.
