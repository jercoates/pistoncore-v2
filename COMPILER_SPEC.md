...
- **Jinja2 everywhere:** all emission goes through editable template files + JSON maps in
  ...
  - **SCOPED (DECISION — Jeremy, 2026-08-01).** The test is HA's MOVING TARGET, not
    tidiness: *"these compilers need to be editable for a user to update as HA does its
    change-the-names-and-how-it-works things, so anything that hits that moving target
    needs to be in templates. The things that don't face that problem can be hard-coded."*
    So anything tracking HA — service names, template functions/filters, how a value is
    read — MUST be template/data and user-editable. Compiler internals not exposed to HA
    churn (control flow, IR shapes, routing decisions) may stay in code. Rationale is the
    relief valve: a user must be able to repair their own compiler with nobody upstream.
  - **AUDIT 2026-08-01 — the rule is currently BROKEN, and it predates the audit.**
    CLEAN: every HA *service* name comes from `webcore_vocab.json`; the automation/script
    skeletons are templates (2 files per band). VIOLATING: the HA *template expressions* —
    every `{{ states('sensor.x') == 'on' }}` and every value read — are Python string code.
    `expression.py` 22 occurrences plus `_JINJA_FUNCS` (~line 619); `emit_yaml.py` 17;
    `resolve.py` 5 (`read_expr`); `emit_pyscript.py` 1. Only the outer skeleton is
    templated — everything INSIDE the automation is generated by Python.
    **RE-MEASURED 2026-08-06 — the scale here was overstated.** Actual counts are
    `expression.py` 11, `emit_yaml.py` 22, `resolve.py` 5, `emit_pyscript.py` 1 (~39,
    not 45), and **`_JINJA_FUNCS` is 24 entries, not 109** (expression.py:600-625).
    109 is the number of webCoRE FUNCTIONS in the vocab, which is a different thing —
    and the PyScript half of those already lives in a band template
    (`templates/compiler/pyscript/2.x/expr_runtime.py.j2`), so it is already compliant.
    The remaining job is the YAML band's 24 lambdas plus the condition/value
    expressions — materially smaller than this audit reads.
    **This is not cosmetic.** The rule exists once but is hand-written twice, once per
    band, and the two copies disagree: webCoRE joins adjacent operands as text for strings
    and multiplication for numbers (groovy :10700-10724); the PyScript side is correct and
    the Jinja side emitted MULTIPLICATION for a text message, compiling a Carbon Monoxide
    alert to `0.0` instead of the detector list. Caught by reading the output, not by any
    test. Fix is staged in `SESSION_BRIEF_ONE_READER_ONE_WRITER.md`.
