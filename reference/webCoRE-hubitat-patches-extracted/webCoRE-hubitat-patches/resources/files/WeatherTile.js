...
function setWeatherTile(weat) {

    weat.forEach((value, key) => {
        ...
        switch (key) {
            case 'current_temperature':
                ...
                break;
            case 'forecast_high':
                out = options.display.forecast_high;
                val = getString(value, out.unit);
                val = isNaN(val) ? val : val.toFixed(out.decimal);
                setValue(val, key);
                break;
            ...
            case 'current_dewpoint':
                ...
                if (options.show_dewpoint) {
                    let dewpoint_text = getDewPointS(dewPoint_f);
