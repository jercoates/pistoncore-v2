...
## 3.5 The helper package — reconciled, not bookkept (DECISION, Jeremy 2026-08-18)

A piston variable that outlives one automation run needs somewhere in HA to live:
an `input_boolean`/`input_number`/`input_text`/`input_datetime`, or a `binary_sensor`
group for a device group. They all go in ONE file,
`pistoncore_packages/pistoncore_variables.yaml`, written on every deploy.

**Every deploy REBUILDS the whole file** from each piston's recorded helper set in
`compile_status.json`. That is deliberate: a deleted piston has no record, so its
helpers stop being written and vanish on reload — reconciling beats bookkeeping,
because there is no orphan list to drift.

**The failure it had.** A rebuild read *"I have no compile record for that piston"*
as *"that piston needs no helpers"*. Those are different things. Deploying from an
instance whose status file does not know every piston therefore deleted the rest:
**34 helpers on 2026-08-09**, including the `binary_sensor.pistoncore_*_cave_motion`
groups three Cave automations used as their turn-off trigger. Reachable three ways —
a second instance against the same HA, a lost or reset `compile_status.json`, or
restoring pistons without it.

**It was silent, which is what made it expensive.** The YAML still compiled, HA still
validated it, the automation still loaded. Only the runtime behaviour broke, and the
loss was found weeks later by a light that stopped turning off. Same failure shape as
the fan that accepted a speed and did not move (HA_LIMITATIONS §3).

**Three-part decision:**

1. **Merge when the records are visibly incomplete.** Compare the pistons the status
   file knows against the pistons on disk; if any are missing, keep what the deployed
   package already has, add what this compile needs, and delete nothing. The package
   file is the only place a forgotten piston's helpers still exist, so
   `helpers.parse_package()` reads them back out of it. **The healthy case is
   unchanged** — records covering every piston reconcile exactly as before, stale
   orphans included.
2. **Two rotating backups** of `compile_status.json`, written before each update.
3. **Diff against the backup and say what went missing** (Jeremy's addition, and the
   part that carries the most weight — the recovery already existed, the *detection*
   did not):

   > `WARNING: 1 helper(s) present at the last deploy are gone now
   > (input_number.pistoncore_pA_count) — if that was not intended, run Recompile All`

   §8's Recompile All is the repair: it rebuilds every helper definition from the
   pistons themselves. The warning exists so somebody knows to press it.

**Accepted limit (Jeremy, 2026-08-18):** a dropped helper loses its VALUE — it returns
at its default. Not worth engineering around; values do not survive a reboot or a
recompile either, and rebuild on the next run. Definitions are what must not be lost.
