...
def compile_yaml(piston: dict, piston_id: str, piston_name: str,
    ...
    # A RECOGNISED IDIOM SHORT-CIRCUITS THE BRANCH PIPELINE (2026-08-13).
    # One idiom in, one HA construct out: when the piston IS a known shape, the
    # automation is written from that shape and never touches the branch IR
    # below, so webCoRE's statement structure has no route into the YAML. An
    # unrecognised piston falls through to the existing path unchanged, which
    # is why this can only add, never regress.
    if emit_intent.enabled():
        _idiom = emit_intent.daily_time(piston, piston_id, piston_name, resolver)
        if _idiom:
            _hdr = (f"# Generated by PistonCore — piston \"{piston_name}\" ({piston_id})\n"
                    f"# DO NOT EDIT — changes are overwritten on recompile. "
                    f"Idiom: daily_time (yaml/classic)\n")
            return {"target": "yaml", "yaml": _hdr + _idiom["yaml"],
                    "reasons": [], "auto_ids": _idiom["auto_ids"],
                    "idiom": "daily_time",
                    "unresolved": resolver.unresolved,
                    "media_warnings": resolver.media_warnings,
...
def _condition(cond: dict, resolver: Resolver, ctx: dict) -> dict:
    ...
    # ── HA'S OWN WORD FOR "THIS ENTITY IS IN THIS STATE" ────────────────────
    # Everything below renders into one generic Jinja template, because that is
    # webCoRE's shape -- left operand, comparison, right operand -- and it fits
    # every case. The result WORKS and is foreign: a census of the corpus found
    # 152 template conditions and ZERO `condition: state`, with 75 of those
    # templates being literally `{{ states('x') == 'y' }}` (2026-08-10, and it
    # matches Jeremy's own research). It matters because the emitted YAML is
    # meant to outlive PistonCore: HA draws a native state condition as an
    # editable row in its own editor, and a template as an opaque blob.
    #
    # Deliberately narrow. `condition: state` means "ALL of these entities are
    # in this state", so it is only equivalent when there is one entity or the
